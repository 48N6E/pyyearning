...
**执行者**:      %s
**SQL语句**:     %s
**确认执行**:    [执行](http://yearning.hgj.net:7000/excute/%s/%s)     [驳回](http://yearning.hgj.net:7000/reject/%s/%s)
====== END ======
""" 
        ...
        if status == 2: #1表示已经执行，2表示未执行，status=4表示工单执行失败            
            ...
            data= {
                ...
                "markdown": {
                    ...
                    str(date),str(executor)," ".join(sql_result),token,str(id),str(token),str(id))
                    }
                }
            return data
